...
if (move_uploaded_file($_FILES['file']['tmp_name'], iconv("utf-8", "big5", $uploadfile)))
	...
	for ($i=1; $i < $lastno; $i++)
		...
		$data_array['AnsD'] = return_between($fraction, '(D)', '＊', EXCL);
		
		
		if (false !== strpos($fraction, $str_reason))
			{
			$data_array['Diffcult'] = return_between($fraction, '＊', '解 析', EXCL);
			$data_array['Analysis'] = return_between($fraction, '解 析', $i+1 .'.（', EXCL);
			}
		else $data_array['Diffcult'] = return_between($fraction, '＊', '解 析', EXCL);
